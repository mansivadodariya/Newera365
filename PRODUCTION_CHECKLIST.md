...
- [ ] OpenNext build deploys clean (`wrangler deploy`) to a `*.workers.dev` preview; smoke-tested
...
- [ ] `npm i -D @opennextjs/cloudflare wrangler` in `apps/web`
...
- [ ] Add `apps/web/open-next.config.ts` (default `defineCloudflareConfig()`)
