...
- **`apps/cms`** — Payload CMS **v2** (not v3) running on a custom Express server (`src/server.ts`). Config is `src/payload.config.ts`. Database is PostgreSQL via `@payloadcms/db-postgres` (Neon in production). 12 collections in `src/collections/` + a `SiteSettings` global in `src/globals/`. Custom non-CMS REST routes (`/api/mt5/*`, `/api/newsletter/*`, `/api/contact`, etc.) are registered in `src/endpoints/index.ts` — most are `501` stubs awaiting Phase 3. Localized content uses Payload's `localization` config (`en`/`ar`, `fallback: true`) with `localized: true` on individual fields.
