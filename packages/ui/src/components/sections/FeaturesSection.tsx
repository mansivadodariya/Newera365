...
import { RevealDemo } from '../motion/RevealDemo';
        ...
        <Spotlight size={460} className="px-5 py-14 xl:py-20">
          <div className="mx-auto max-w-[390px] md:max-w-2xl xl:max-w-[1200px]">
            ...
            <div className="xl:flex xl:items-end xl:justify-between xl:gap-16">
              ...
              <p className="font-body text-lead mt-4 max-w-[440px] text-white xl:mt-0 xl:flex-shrink-0">
                {t('whySubtitle')}
              </p>
            </div>
                  ...
                  {resolvedMetrics.map(({ key, value, title, desc }, i) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="font-body text-body mt-1.5 max-w-[36ch] text-white transition-colors duration-300">
                          {desc}
                        </p>
                      </div>
                    </div>
            ...
            <p className="font-body text-caption mt-2 text-white">{t('whyMetricsNote')}</p>

            {/* USP proof strip */}
            <div className="mt-10 border-t border-white/10 pt-8 xl:mt-14">
              <p className="text-eyebrow font-mono font-medium uppercase text-white">
                {t('uspHeading')}
              </p>
              <ul className="mt-5 grid grid-cols-1 gap-x-8 gap-y-3.5 sm:grid-cols-2 xl:grid-cols-3">
                {USPS.map((key) => (
                  <li key={key} className="flex items-start gap-3">
                    ...
                    <span className="font-body text-body font-medium text-white">{t(key)}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-9 flex flex-wrap items-center gap-3">
                ...
                <Link
                  ...
                  className="font-body hover:border-accent-bright/60 inline-flex flex-none items-center gap-1.5 rounded-full border border-white/15 px-6 py-4 text-[15px] font-medium text-white transition-colors"
                >
                  {t('whyCtaCompare')}
                  <svg
                    width="13"
