...
export interface AnalysisSymbolOption {
  id: string;
  label: string;
  tvSymbol: string;
}

export const ANALYSIS_SYMBOLS: AnalysisSymbolOption[] = [
  { id: 'eurusd', label: 'EUR/USD', tvSymbol: 'OANDA:EURUSD' },
  { id: 'gbpusd', label: 'GBP/USD', tvSymbol: 'OANDA:GBPUSD' },
  { id: 'usdjpy', label: 'USD/JPY', tvSymbol: 'OANDA:USDJPY' },
  { id: 'audusd', label: 'AUD/USD', tvSymbol: 'OANDA:AUDUSD' },
  { id: 'usdcad', label: 'USD/CAD', tvSymbol: 'OANDA:USDCAD' },
  { id: 'xauusd', label: 'XAU/USD — Gold', tvSymbol: 'OANDA:XAUUSD' },
  { id: 'btcusd', label: 'BTC/USD — Bitcoin', tvSymbol: 'BITSTAMP:BTCUSD' },
  { id: 'ethusd', label: 'ETH/USD — Ethereum', tvSymbol: 'BITSTAMP:ETHUSD' },
];
...
// Desk container: full width up to max desktop bounds.
const DESK = 'mx-auto w-full max-w-[1200px]';
...
export function AnalystChartPage({ cmsCalls, cmsAnalyst, locale }: AnalystChartPageProps = {}) {
  ...
  // Symbol selection states for Technical Analysis Gauge & Symbol Info
  const [gauge1Symbol, setGauge1Symbol] = useState('OANDA:EURUSD');
  const [infoSymbol, setInfoSymbol] = useState('OANDA:EURUSD');
  ...
  return (
    <>
      ...
      {/* --------------------------------------------------------------------------- */}
      {/* Interactive Market Analysis Suite (Matching Reference Specs) */}
      {/* --------------------------------------------------------------------------- */}

      {/* 1. Technical Analysis Gauges */}
      <section className="px-5 pb-12">
        <ScrollReveal className={DESK}>
          <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div>
              <SectionKicker className="mb-2">TECHNICAL INDICATORS</SectionKicker>
          ...
          <div className="overflow-hidden rounded-[20px] border border-white/10 bg-black p-5 shadow-xl">
            <div className="mb-4 flex flex-col gap-3 border-b border-white/10 pb-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <span className="font-mono text-xs font-semibold uppercase tracking-wider text-[#00b050]">
                  Instrument Indicator
                </span>
                <h3 className="mt-0.5 font-sans text-[20px] font-bold text-white">
                  {ANALYSIS_SYMBOLS.find((s) => s.tvSymbol === gauge1Symbol)?.label ?? 'EUR/USD'}
                </h3>
              </div>
              <div className="flex items-center gap-3">
                <span className="font-sans text-[12px] font-semibold uppercase tracking-wider text-gray-400">
                  Select Symbol:
                </span>
                <select
                  value={gauge1Symbol}
                  onChange={(e) => setGauge1Symbol(e.target.value)}
                  className="cursor-pointer rounded-lg border border-[#00b050]/40 bg-[#161616] px-4 py-2 font-sans text-[14px] font-bold text-white outline-none focus:border-[#00b050]"
                >
                  {ANALYSIS_SYMBOLS.map((s) => (
                    <option key={s.id} value={s.tvSymbol} className="bg-[#111] text-white">
                      {s.label}
                    </option>
                  ))}
                </select>
            ...
            <div className="h-[440px] w-full overflow-hidden md:h-[480px] [&_iframe]:overflow-hidden">
              <ChartWidget
                key={gauge1Symbol}
                ...
                height="100%"
                interactive={true}
                config={{ showIntervalTabs: true, displayMode: 'multiple', isTransparent: false }}
              />
            </div>
          </div>
        </ScrollReveal>
      ...
      {/* 2. Symbol Details & Watchlist */}
      <section className="px-5 pb-12">
        <ScrollReveal className={DESK}>
          <div className="mb-6">
            <SectionKicker className="mb-2">MARKET INSTRUMENTS</SectionKicker>
            <h2 className="text-foreground text-title font-sans text-[22px] font-bold md:text-[26px]">
              Symbol Details
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            <div className="overflow-hidden rounded-[20px] border border-white/10 bg-black p-4 shadow-xl">
              <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <h3 className="font-sans text-[16px] font-bold text-white">Symbol Info</h3>
                <div className="flex items-center gap-2">
                  ...
                  <select
                    value={infoSymbol}
                    onChange={(e) => setInfoSymbol(e.target.value)}
                    className="cursor-pointer rounded-lg border border-[#00b050]/40 bg-[#161616] px-3 py-1.5 font-sans text-[13px] font-bold text-white outline-none focus:border-[#00b050]"
                  >
                    {ANALYSIS_SYMBOLS.map((s) => (
                      <option key={s.id} value={s.tvSymbol} className="bg-[#111] text-white">
                        {s.label}
            ...
            <div className="overflow-hidden rounded-[20px] bg-black p-3.5 shadow-xl">
              <h3 className="mb-3 font-sans text-[16px] font-bold text-white">
                Watchlist & Quotes
              </h3>
              <div className="h-[265px] min-h-[260px] w-full overflow-hidden rounded-[14px]">
                <ChartWidget
                  type="market-quotes"
                  theme="dark"
                  width="100%"
      ...
      {/* 3. Currency Analysis (Heatmap & Cross Rates) */}
      <section className="px-5 pb-12">
        <ScrollReveal className={DESK}>
          <div className="mb-6">
            <SectionKicker className="mb-2">FX DESK MATRIX</SectionKicker>
            <h2 className="text-foreground text-title font-sans text-[22px] font-bold md:text-[26px]">
              Currency Analysis
            </h2>
          </div>
          ...
          <div className="flex w-full min-w-0 flex-col gap-6">
            <div className="w-full min-w-0 overflow-hidden rounded-[20px] bg-black p-3.5 shadow-xl">
              <h3 className="mb-3 font-sans text-[16px] font-bold text-white">Currency Heatmap</h3>
              <div className="h-[460px] min-h-[420px] w-full overflow-hidden rounded-[14px] md:min-h-[460px]">
                <ChartWidget
                  type="forex-heat-map"
                  theme="dark"
                  width="100%"
      ...
      <section className="px-5 pb-12">
        <ScrollReveal className={DESK}>
          <div className="mb-6">
            <SectionKicker className="mb-2">MACROECONOMIC FEED</SectionKicker>
            <h2 className="text-foreground text-title font-sans text-[22px] font-bold md:text-[26px]">
              Economic Events
            </h2>
          </div>
          ...
          <div className="w-full min-w-0 overflow-hidden rounded-[20px] bg-black p-3.5 shadow-xl">
            <div className="h-[560px] min-h-[480px] w-full overflow-hidden rounded-[14px] md:min-h-[560px]">
              <ChartWidget
      ...
      {/* 5. Market News (Top Stories) */}
      <section className="px-5 pb-14">
        <ScrollReveal className={DESK}>
          <div className="mb-6">
            <SectionKicker className="mb-2">NEWS DESK</SectionKicker>
            <h2 className="text-foreground text-title font-sans text-[22px] font-bold md:text-[26px]">
              Market News
            </h2>
          </div>
          ...
          <div className="w-full min-w-0 overflow-hidden rounded-[20px] bg-black p-3.5 shadow-xl">
            <div className="h-[640px] min-h-[560px] w-full overflow-hidden rounded-[14px] md:min-h-[640px]">
              <ChartWidget
                type="timeline"
                theme="dark"
                width="100%"
                height="100%"
                interactive={true}
                config={{ feedMode: 'market', market: 'stock', displayMode: 'regular' }}
              />
            </div>
          </div>
        </ScrollReveal>
