...
export function AccountsPage({ cmsAccounts }: AccountsPageProps) {
  ...
  const matrixAccounts = uniqueCmsAccounts
                  ...
                  {[row.std, row.raw, row.vip].slice(0, matrixColNames.length).map((val, j) => (
                    ...
                    >
                      {val ? (
                        ...
                        <span className="flex h-6 w-6 items-center justify-center rounded-full border border-red-500/30 bg-red-500/15 shadow-sm">
                          <Cross />
                        </span>
                      )}
                    </div>
