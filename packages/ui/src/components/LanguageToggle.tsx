...
export function LanguageToggle({ fullWidth = false }: LanguageToggleProps) {
  ...
  return (
    <button
      ...
      aria-label={ariaLabel}
      title={ariaLabel}
      className="font-body text-foreground dark:bg-surface flex h-[38px] min-w-[38px] flex-shrink-0 items-center justify-center gap-1.5 rounded-xl bg-[#f4f4f5] px-2.5 text-[12px] font-semibold tracking-wide transition-[background-color,transform] hover:bg-accent/[0.10] active:scale-[0.94] dark:hover:bg-accent/[0.10]"
    >
      {/* Globe icon */}
